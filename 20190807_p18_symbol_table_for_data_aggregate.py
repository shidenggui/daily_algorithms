...
class Node:
    token: Token
    children: List["Node"]
    prev: "Node" = dataclasses.field(default=None, repr=False)
...
class PrintVistor:
    ...
    def print_struct_decl(self, node: Node):
        print(" " * self.indent + "struct ", end="")
        self.print(node.children[0])
        print("\n" + self.indent * " " + "{")
        self.incr_indent()
        for child in node.children[1:]:
    ...
    def print_field_decl(self, node: Node):
        print(" " * self.indent, end="")
        self.print(node.children[0])
        print(" ", end="")
        self.print(node.children[1])
...
class ScopeVisitor:
    ...
    def visit_struct_decl(self, node: Node):
        print(f"line {node.token.line}: def struct {node.children[0].token.text}")
        symbol = StructSymbol(
        ...
        print(f"fields: {self.current_scope.symbols}")
        self.current_scope = self.current_scope.get_enclosing_scope()

    def visit_field_decl(self, node: Node):
        self.current_scope.define(
            Symbol(type=node.children[0].token.text, name=node.children[1].token.text)
        )

    def visit_dot(self, node: Node):
        struct_symbol = self.visit(node.children[0])
        self.visit(node.children[1])

        left = node.children[0]
        right = node.children[1]
        if left.token.type == TokenType.ID:
            symbol = self.current_scope.resolve(left.token.text)
            print(f"line {node.token.line}: ref {right.token.text} = {symbol}")
            struct_symbol: StructSymbol = self.current_scope.resolve(symbol.type)

        symbol = struct_symbol.resolve(right.token.text)
        print(
            f"line {node.token.line}: ref {struct_symbol.get_name()}.{symbol.name} = {symbol}"
        )
        return struct_symbol.resolve_member(symbol.type)
    ...
    def visit_var_decl(self, node: Node):
        ...
        self.visit(node.children[1])
        self.current_scope.define(Symbol(name=var_name, type=type))
    ...
    def visit_func_decl(self, node: Node):
        ...
        self.current_scope.define(Symbol(name=func_name, type=type))
...
NODE_MAP = {
    ...
    TokenType.FIELD_DECL: Node,
    TokenType.STRUCT_DECL: Node,
}
...
class MethodSymbol(Scope, Symbol):
    ...
    def __init__(self, name=None, type=None, prev: "Scope" = None, line=None):
        self.name = name
        self.type = type
        if prev:
            prev.next = self
...
class StructSymbol(Scope, Symbol):
    """结构体类型
    作为 struct 是为函数参数存在。
    """
    ...
    def get_name(self):
        name = [self.name]
        prev = self.prev
        while prev and isinstance(prev, Symbol):
            name.append(prev.name)
            prev = prev.prev
...
class Parser:
    ...
    def append_node(token_type):
        def decorator(f):
            @functools.wraps(f)
            def wrapper(self, *args, **kwargs):
                node = Node(Token(token_type, self.LL(1).text, self.LL(1).line), [])
                self.ast.children.append(node)
                node.prev = self.ast
        ...
        while True:
            if self.LL(1).type == TokenType.TYPE:
                ...
                if self.LL(2).type == TokenType.LBRACK:
                    self.call_func()
                elif self.LL(2).type == TokenType.ID:
                    self.var_dec()
                else:
                    self.assign()
            elif self.LL(1).type == TokenType.STRUCT:
                self.struct_decl()
            else:
                break
    ...
    @node(TokenType.STRUCT_DECL)
    def struct_decl(self):
        self.match(TokenType.STRUCT)
        self.id()
        self.match(TokenType.LCURLY)
        while self.LL(1).type in {TokenType.TYPE, TokenType.STRUCT, TokenType.ID}:
            if self.LL(1).type == TokenType.STRUCT:
                self.struct_decl()
            else:
                self.field_decl()
    ...
    def call_func(self):
        self.id()
        self.match(TokenType.LBRACK)
        while self.LL(1).type in {TokenType.ID, TokenType.NUM, TokenType.LBRACK}:
            self.primary()
            if not self.LL(1).type == TokenType.RBRACK:
                self.match(TokenType.COMMA)
        self.match(TokenType.RBRACK)
    ...
    def add(self):
        ...
        while self.LL(1).type == TokenType.ADD:
            ...
            node = AddNode(add_token, [node], prev=saved)
            node.children[0].prev = node
            self.ast = node
        self.ast = saved
    ...
    def member(self):
        node = IdNode(self.LL(1), [])
        self.match(TokenType.ID)
        while self.LL(1).type == TokenType.DOT:
            node = Node(self.LL(1), [node, IdNode(self.LL(2), [])])
